...
def redirect_oauth(current_user: User):
    ...
    refreshed_token, expired_in = linkedin.refresh_token(auth_code)
    linkedin_info = linkedin.user_info(refreshed_token)
    res = requests.post("http://localhost:5000/account",
                        data=json.dumps(
                            {"auth_code": str(refreshed_token), "expired_in": expired_in,
                             "type": MediaType.LINKEDIN.value, "info": linkedin_info}),
...
def redirect_oauth_twitter(current_user: User):
    ...
    # res = requests.post("http://localhost:5000/account",
    #                     data=json.dumps(
    #                         {"auth_code": str(auth_code), "expired_in": expired_in,
    #                          "type": MediaType.TWITTER.value, "info": linkedin_info}),
    #                     headers={'Content-Type': 'application/json'})
    #
    # res.raise_for_status()
